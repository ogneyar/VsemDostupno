...
function requestMessage($bot, $message, $master, $admin) {
    ...
    *********************/
    if ($text == "Помощь" || $text == "/help" || $text == "Назад в Помощь")
    {
    ...
    ************************/
    if ($text == "Кооперация" || $text == "/cooperation")
    {
        ...
        $send .= "Несмотря на то, что потребительские кооперативы все время у нас «на слуху», мало кто знает, что они из себя представляют. Максимум, что известно – что это общество, образованное пайщиками, чтобы вместе что-то делать или закупать.\r\n\r\n";
        $send .= "И в основном все вопросы потребкооперации рассматриваются именно через призму пайщиков – как обменять пай, как правильно оформлять и платить взносы, переходит ли пай по наследству, и так далее.\r\n\r\n";
        $send .= "Отрицать пользу и выгодность работы потребительских обществ для пайщиков, конечно, нельзя. Но те возможности, которые открываются перед организаторами, способны серьезно поменять Ваши представления о бизнесе.";
    ...
    *******************/
    if ($text == "/newbie" || $text == "Новичкам" || $text == "/new" || $text == "Привязать телефон к л/к")
    {    
        $send = "Если Вы сменили телефон, для управления л/к Вам необходимо привязать новый";
        ...
        $send = "Узнайте № id, нового телефона и отправьте его администратору для подключения";
